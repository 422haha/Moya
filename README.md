...
- **Paging3**: 백엔드 서버에서 불러오는 전체 리스트 데이터를 나눠서 호출할 수 있습니다.
- **Retrofit2**: 백엔드 서버와의 HTTP 요청을 인터페이스 메서드로 매핑하여, 네트워크 요청 로직을 직관적이고 쉽게 작성할 수 있습니다.
- **Jetpack Compose Navigation**: 안드로이드 Compose 환경에서 네비게이션을 동작할 수 있습니다.
- **FCM**: 사용자가 정해진 시간에 강의 알림을 받을 수 있도록 Firebase를 활용하여 푸시 알림을 받을 수 있게 하였습니다.
