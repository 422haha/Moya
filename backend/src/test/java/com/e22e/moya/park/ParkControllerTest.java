...
public class ParkControllerTest {
    ...
    void testGetNearestPark() {
        ...
        // 로그 출력 - Given 단계
        System.out.println("Given 단계 - token: " + token + ", latitude: " + latitude + ", longitude: " + longitude + ", userId: " + userId);

        // When
        ResponseEntity<Map<String, Object>> responseEntity = parkController.getNearestPark(token, latitude, longitude);
    ...
    void testGetParks() {
        ...
        when(jwtUtil.getUserIdFromToken(token)).thenReturn(userId);
        when(parkService.getParks(userId, latitude, longitude, page, size)).thenReturn(parkListResponseDto);

        // 로그 출력 - Given 단계
        System.out.println("Given 단계 - token: " + token + ", latitude: " + latitude + ", longitude: " + longitude + ", userId: " + userId + ", page: " + page + ", size: " + size);

        // When
        ResponseEntity<Map<String, Object>> responseEntity = parkController.getParks(token, page, size, latitude, longitude);
    ...
    void testGetParkDetail() {
        ...
        Long parkId = 1L;
        Long userId = 1L;
        ParkDetailResponseDto parkDetailResponseDto = new ParkDetailResponseDto(1L, "싸피 뒷뜰", "싸피 기숙사 뒷뜰", "image_url", null);

        when(jwtUtil.getUserIdFromToken(token)).thenReturn(userId);
        when(parkService.getParkDetail(parkId)).thenReturn(parkDetailResponseDto);

        // 로그 출력 - Given 단계
        System.out.println("Given 단계 - token: " + token + ", parkId: " + parkId + ", userId: " + userId);

        // When
        ResponseEntity<Map<String, Object>> responseEntity = parkController.getParkDetail(token, parkId);
